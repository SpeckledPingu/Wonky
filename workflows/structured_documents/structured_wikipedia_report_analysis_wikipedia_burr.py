...
# from Wikipedia articles. It is intended to be used as a detailed guide or prompt
...
if __name__ == "__main__":
    ...
    # 1. Obtain the text of a Wikipedia article.
    #    Example:
    #    crs_report_actual_text = "" # Load your specific Wikipedia article text here
    ...
    #    This prompt would include the Wikipedia article text and the
...
# from Wikipedia articles (or similar analytical documents). It is intended to be used
...
def format_wiki_sections_w_citations(wiki_document, base_citation):
...
def format_wikipedia_page(wiki_document):
...
#%%
def insert_extraction_data(extractions, cursor, conn, run_id, project_id, run_timestamp, subject_matter, focus,
                           analysis_type, source_dataset):
    project_run_columns = [
    ...
    overview_columns = [
        'run_id', 'run_timestamp', 'project_id', 'subject_matter', 'focus', 'analysis_type', 'overview',
        'overview_citations', 'overview_type', 'overview_pages', 'source_document', 'source_dataset'
    ]

    overview_sources_columns = ['run_id', 'run_timestamp', 'project_id', 'subject_matter', 'focus', 'analysis_type',
                                'text', 'citation', 'source_document', 'source_dataset']

    for document_id, _extraction in extractions.items():
        research_run_insert_sql = insert_research_metadata(_extraction, run_id, project_id, run_timestamp,
                                                           subject_matter, focus, analysis_type, document_id,
                                                           source_dataset, project_run_columns)
        cursor.execute(research_run_insert_sql[0], research_run_insert_sql[1])

        overview_insert_sql = insert_overview_metadata(_extraction, run_id, project_id, run_timestamp, subject_matter,
                                                       focus, analysis_type, document_id, source_dataset,
                                                       overview_columns)
        cursor.execute(overview_insert_sql[0], overview_insert_sql[1])

        for _cite_source in _extraction['overview_cite_sources']:
            overview_sources_insert_sql = insert_research_sources_metadata(_cite_source, run_id, project_id,
                                                                           run_timestamp, subject_matter, focus,
                                                                           analysis_type, document_id, source_dataset,
                                                                           overview_sources_columns)
...
#%%
def format_enriched_report(extracted_article, full_document_data, report_sections, overview_type):
    article_pages = extracted_article.split('---')
    # article_metadata = deepcopy(full_document_data)
    article_metadata = full_document_data.iloc[0].to_dict()
    # print(article_pages)
    print(len(article_pages))
    print(article_metadata.keys())
    print(type(full_document_data))
    print(full_document_data.head())
...
#%%
@action(reads=[], writes=["article_ids","subject_matter","focus"])
def wikipedia_search_reports(state: State, focus: str, subject_matter: str, num_results: int) -> State:
    ...
    print(article_ids)
    return state.update(article_ids=article_ids, subject_matter=subject_matter, focus=focus)
...
def retrieve_and_format_full_article(state: State, analysis_prompts: dict) -> State:
    ...
    for article_id in tqdm(state['article_ids']):
        wikipedia_page = retrieve_wikipedia_page(article_id)
        title_number = generate_numeric_hash(wikipedia_page['title'])
        wiki_base_citation = wikipedia_page['citation']

        wiki_document = format_wiki_sections_w_citations(wikipedia_page['sections'],
                                                         wikipedia_page['citation'])
        full_document = format_wikipedia_page(wiki_document)
        full_document = f"""**Title**: {wikipedia_page['title']}

**Report ID:** {wiki_base_citation}

**Publication Date:**: {datetime.now().strftime('%Y-%m-%d')}

{full_document}"""

        wikipedia_page['sections'] = wiki_document
        wikipedia_page['text'] = full_document
        all_report_sections[wiki_base_citation] = wikipedia_page['sections']
        ...
        all_prompts[wiki_base_citation] = formatted_prompt
        all_document_data[wiki_base_citation] = wikipedia_page
...
def structured_wikipedia_report_build():
    extraction_app = (
        ApplicationBuilder()
        .with_actions(
            wikipedia_search_reports,
        ...
        .with_transitions(
            ("wikipedia_search_reports", "retrieve_and_format_full_article"),
        ...
        .with_entrypoint("wikipedia_search_reports")
        .with_tracker(
            "local",
...
# #%%
# _ = retrieve_full_document('Chemistry')
# #%%
# _.keys()
#%%
